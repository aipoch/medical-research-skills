...
> 💡 **Some sources need pre-processing before any skill can act.** A PDF with a real text layer, structured data, or tabular content feeds straight into the Document Production or Computational Pathology skills. A paper that only exists as a **scanned PDF** (no text layer), a conference talk **recording** (audio), or a **web-only preprint** has no extractable text — those stall before the pipeline starts. Run them through [cue-omni-reader](https://github.com/sensedeal/cue-skills/tree/main/cue-omni-reader) first to get Markdown — web pages including in-page video/attachments, plus authorized local documents, audio, or video; several local files at once — then feed the resulting text into any skill above. Install: `npx skills add sensedeal/cue-skills --skill cue-omni-reader` (MIT; may bill).
