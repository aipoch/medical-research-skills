...
> 💡 **Every skill above assumes the source is already text.** A literature PDF with a real text layer, a structured PubMed search, tabular data — those feed straight in. A paper that only exists as a scan, a conference talk recording, or a web-only preprint has no text layer to extract. Run those through [cue-omni-reader](https://github.com/sensedeal/cue-skills/tree/main/cue-omni-reader) first to get Markdown — web pages including in-page video/attachments, plus authorized local documents, audio, or video; several local files at once — then feed the resulting text into any of the skills above. Install: `npx skills add sensedeal/cue-skills --skill cue-omni-reader` (MIT; may bill).
